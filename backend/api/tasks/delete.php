...
if (!empty($data->id) && !empty($data->user_id)) {
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            echo json_encode(array(
                "status" => "error",
                "message" => "không tìm thấy công việc để xóa hoặc bạn không có quyền xóa công việc này!"
            ));
        }
    ...
    echo json_encode(array("status" => "error", "message" => "Thiếu thông tin id hoặc user_id."));
}